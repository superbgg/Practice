...
%>		
	...
	<input type="hidden" name="b_idx" value="<%=b_idx%>"> 
		...
		<p><%=session.getAttribute("userid")%> : <input  type="text" name="re_content" size="40">
		<input type="submit" value="확인">
		</p>
	</form>
	<hr/>
<%
	sql= "select re_idx, re_userid, re_content, re_regdate from tb_reply where re_boardidx=? order by re_idx asc";
	pstmt = conn.prepareStatement(sql);
	pstmt.setString(1, b_idx);
	rs = pstmt.executeQuery();
